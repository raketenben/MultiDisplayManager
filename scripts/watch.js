...
const setupMainPackageWatcher = (viteDevServer) => {
    ...
    let spawnProcess1 = null;
    let spawnProcess2 = null;
    ...
    return getWatcher({
        ...
        writeBundle() {
            ...
            if (spawnProcess1 !== null) {
                spawnProcess1.kill('SIGINT');
                spawnProcess1 = null;
            }
            if (spawnProcess2 !== null) {
                spawnProcess2.kill('SIGINT');
                spawnProcess2 = null;
            }
            ...
            spawnProcess1.stdout.on('data', d => d.toString().trim() && logger.warn(d.toString(), { timestamp: true }));
            spawnProcess1.stderr.on('data', d => d.toString().trim() && logger.error(d.toString(), { timestamp: true }));

            spawnProcess2.stdout.on('data', d => d.toString().trim() && logger.warn(d.toString(), { timestamp: true }));
            spawnProcess2.stderr.on('data', d => d.toString().trim() && logger.error(d.toString(), { timestamp: true }));
        },
    });
};
